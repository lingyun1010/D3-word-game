...
//part2
  ...
  function updateGame() {
    ...
    if (e.node().tagName === "LI" && !e.classed("disabled")) {
      ...
      e.classed("disabled");
      e.text(guess + " --> Matching Letters: " + similarityScore) ;
      setGuessCount(guessCount - 1);

      // check whether the game is over
      if (similarityScore === password.length) {
        d3.select("#winner").classed( 'hide', !d3.select("#winner").classed('hide'));
        d3.select(this).on('click', null);
      } else if (guessCount === 0) {
        d3.select("#loser").classed( 'hide', !d3.select("#winner").classed('hide'));
        d3.select(this).on('click', null);
      }
    }
  }
